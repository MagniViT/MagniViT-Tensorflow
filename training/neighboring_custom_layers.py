...
class NeighborAggregator(Layer):
    ...
    def get_affinity(self, Idx):
        ...
        rows = tf.experimental.numpy.ravel(
            tf.map_fn(
                fn=lambda t: tf.repeat(t, repeats=self.k + 1),
                elems=tf.range(tf.shape(Idx)[0]),
            )
        )
        ...
        sparse_matrix = tf.sparse.SparseTensor(
            indices=tf.transpose([rows, columns]),
...
class CHARM(Layer):
    ...
    def call(self, inputs, mask=None):
        ...
        pool_outputs = []
        for k in self.k:
            alpha, a_raw = NeighborAggregator(output_dim=1, k=k, name="alpha")(
                [attention_matrix, sp_matrix]
            )
