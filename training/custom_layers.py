...
from tensorflow.keras.layers import Layer,LayerNormalization,Conv2D,ZeroPadding2D, Dense
...
def moore_penrose_iter_pinv(x, iters=6):
    ...
    z = tf.einsum(transpose_pattern,x)/ (tf.math.reduce_max(col) * tf.math.reduce_max(row))
...
class NystromAttention(tf.keras.layers.Layer):
    def __init__(
        ...
        if residual:
            ...
            self.res_conv = tf.keras.models.Sequential([
                tf.keras.layers.ZeroPadding2D(padding=(padding, 0), data_format="channels_first"),
                tf.keras.layers.Conv2D(filters=self.heads, kernel_size=(kernel_size, 1), padding="valid",
                                       groups=heads, data_format="channels_first",name='conv_attn')
            ]
            )
    ...
    def call(self, input_tensor, mask=None):
        ...
        input_tensor = tf.cond(tf.greater(remainder, 0), padded_matrix, lambda: tf.identity(input_tensor))
        ...
        attn1, attn2, attn3 = map(lambda t: tf.nn.softmax(t, axis=-1), (sim1, sim2, sim3))
...
class PPEG(Layer):
    def __init__(self, dim=512):
        ...
        self.proj = tf.keras.models.Sequential([
            ...
            Conv2D(filters=dim, kernel_size=7, padding="valid",strides=1,
                                   groups=dim, data_format="channels_first",name='conv_1')
        ])

        self.proj1 = tf.keras.models.Sequential([
            ZeroPadding2D(padding=5 // 2, data_format="channels_first"),
            Conv2D(filters=dim, kernel_size=5, padding="valid", strides=1,
                   groups=dim, data_format="channels_first",name='conv_2')
        ])

        self.proj2 = tf.keras.models.Sequential([
            ZeroPadding2D(padding=3 // 2, data_format="channels_first"),
            Conv2D(filters=dim, kernel_size=3, padding="valid", strides=1,
                   groups=dim, data_format="channels_first", name='conv_3')
        ])
    ...
    def call(self,input_tensor, mask=None):
        ...
        x = tf.transpose(tf.reshape(x, [-1,  tf.shape(x)[1] ,  tf.shape(x)[2] * tf.shape(x)[3]]), perm=[0,2,1])
...
class TransMIL(Layer):
    def __init__(self, n_classes, seed):
        ...
        self.cls_token = tf.Variable(
            initial_value=w_init(shape=(1, 1, 512), dtype="float32"),
            trainable=True
        )
    ...
    def call(self, inputs, mask=None):
        ...
        _H, _W = tf.cast(tf.math.ceil(tf.math.sqrt(tf.cast(H, tf.float32))),tf.int32), \
