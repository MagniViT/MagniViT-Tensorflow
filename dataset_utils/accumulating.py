...
class Accumulating_task_generator(tf.keras.utils.Sequence):
    def __init__(self, filenames, feature_path, csv_file, train=True, batch_size=1):
        ...
        self.feature_path_20 = os.path.join(
            self.feature_path, "20x_mag/tf_features/h5_files"
        )
        self.feature_path_10 = os.path.join(
            self.feature_path, "10x_mag/tf_features/h5_files"
        )
        self.feature_path_5 = os.path.join(
            self.feature_path, "5x_mag/tf_features/h5_files"
        )
        self.on_epoch_end()
    ...
    def pad_tensor(self, tensor_1, tensor_2):

        div = np.floor(tensor_1.shape[0] / tensor_2.shape[0])
        remainder = tensor_1.shape[0] % tensor_2.shape[0]

        tensor_2 = np.repeat(tensor_2, div, axis=0)
        tensor_2 = np.concatenate(
            [
                tensor_2,
                tensor_2[
                    :remainder,
